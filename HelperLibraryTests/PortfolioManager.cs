...
namespace HelperLibraryTests
{
    ...
    public class PortfolioManager : PortfolioManagerBase
    {
        ...
        /// </summary>
        public PortfolioManager(IStopLossSettings stopLossSettings = null, IPortfolioSettings portfolioSettings = null, IScoringProvider scoringProvider = null)
        ...
        /// <summary>
        /// ermöglicht das Testen des Pms
        /// </summary>
        /// <param name="portfolio"></param>
        internal void PassInTestTransactions(List<TransactionItem> portfolio)
        {
        ...
        public void PassInCandidates(IEnumerable<TradingCandidate> candidates)
        {
            ...
            PortfolioAsof = candidates.FirstOrDefault().Record.Asof;

            //die neuen Candidaten hinzufügen
            _tradingCandidates.AddRange(candidates);

            //die bestehenden einfügen 
            EvaluateCurrentPortfolio();

        }

        internal override void EvaluateCurrentPortfolio()
        {
            _tradingCandidates.AddRange(RankCurrentPortfolio());
            ApplyPortfolioRules();
        }
        ...
        internal override void ApplyPortfolioRules()
        {
            ...
            //allerdings aus effinzienz gründen zuerst das Current Portfolio Evaluieren
            if (_tradingCandidates.Count > 0)
            {
                //check Candidates
                foreach (var candidate in _tradingCandidates.OrderByDescending(x => x.Score))
                {
                    ...
                    //Wenn es kein aktives investmen ist würde das System die Positon neu erwerdben

                    while (HasCash())
                    {
                        //Try Open new Position//
                        OpenNewPosition(candidate);
                    ...
                    //   var scoringDic = GetCurrentPortfolioRanking();
        ...
        private void OpenNewPosition(TradingCandidate candidate)
        {
            if (!_temporaryAdjustmentPortfolio.ContainsKey(AdjustmentType.InitialBuy))
                _temporaryAdjustmentPortfolio.Add(AdjustmentType.InitialBuy, new List<TransactionItem>());

            var targetAmountEur = _portfolioSettings.PortfolioValue * (decimal)_portfolioSettings.MaximumInitialPositionSize;
            var targetShares = (int)Math.Round(targetAmountEur / candidate.Record.AdjustedPrice, 0, MidpointRounding.ToEven);

            var transactionItem = new TransactionItem
            {
                SecurityId = candidate.Record.SecurityId,
                AmountEur = targetAmountEur,
                Shares = targetShares,
                Cancelled = 0,
                TransactionDateTime = PortfolioAsof,
                TransactionType = (int)TransactionType.Open,
                Weight = targetAmountEur / _portfolioSettings.PortfolioValue

            };


            _temporaryAdjustmentPortfolio[AdjustmentType.InitialBuy].Add(transactionItem);

        }

        internal override IEnumerable<TradingCandidate> RankCurrentPortfolio()
        {
            // das aktuelle Portfolio neu bewerten
            foreach (var transactionItem in CurrentPortfolio)
            {
                //das Result vom Scpring Provider abgragen und neuen Candidaten returnen
                var result = _scoringProvider.GetScore(transactionItem.SecurityId, PortfolioAsof);
                yield return new TradingCandidate(_scoringProvider.GetDataRecord(transactionItem.SecurityId, PortfolioAsof), result);
        ...
        internal override void RegisterScoringProvider(IScoringProvider scoringProvider)
        {
            _scoringProvider = scoringProvider;
        }

        internal override bool HasCash()
        {
    ...
    public abstract class PortfolioManagerBase
    {
        ...
        internal IScoringProvider _scoringProvider;
        ...
        /// </summary>
        internal DateTime PortfolioAsof { get; set; }
        ...
        /// </summary>
        internal abstract IEnumerable<TradingCandidate> RankCurrentPortfolio();


        /// <summary>
        /// Die Methode die jeden Tah zur Neubewertung aufgerufen wird
        /// </summary>
        internal abstract void EvaluateCurrentPortfolio();
        ...
        internal abstract void RegisterScoringProvider(IScoringProvider scoringProvider);


        /// <summary>
        /// methode die True zurückgibt, solage noch Cash zur Veranlagun vorhanden ist
        /// </summary>
        /// <returns></returns>
        internal abstract bool HasCash();
    ...
    public class TransactionsWrapper : IEnumerable<TransactionItem>
    {
        ...
        {
            //set the flag so we know we are in Testmode
            Portfolio.TestMode = true;
